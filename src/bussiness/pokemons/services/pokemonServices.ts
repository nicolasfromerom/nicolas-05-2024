...
export const getPokemons = async(offset: number, limit: number): Promise<Pokemons[]> => {

    const { data } = await pokemonService.get<PokemonsListResponse>(`/pokemon?offset=${offset}&limit=${limit}`);
